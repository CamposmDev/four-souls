...
/**
 * No longer used since there's a better handler for fetching the cards
 * @see CardFetcher
 */
@Deprecated
...
//        var val = div.getElementsByTag("p").get(1).text().toLowerCase();
        ...
        if (t.contains("character")) {
            downloadImage(srcImg, CARDS_DIR + CHARACTER_DIR + NAME);
        } else if (t.contains("eternal")) {
            downloadImage(srcImg, CARDS_DIR + ETERNAL_DIR + NAME);
        } else if (t.contains("treasure")) {
            downloadImage(srcImg, CARDS_DIR + TREASURE_DIR + NAME);
        } else if (t.contains("loot")) {
            downloadImage(srcImg, CARDS_DIR + LOOT_DIR + NAME);
        } else if (t.contains("monster")) {
            downloadImage(srcImg, CARDS_DIR + MONSTER_DIR + NAME);
        }  else if (t.contains("room")) {
            downloadImage(srcImg, CARDS_DIR + ROOM_DIR + NAME);
        } else {

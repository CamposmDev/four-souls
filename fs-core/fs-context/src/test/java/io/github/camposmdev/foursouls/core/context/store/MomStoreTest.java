	...
	class UserPlugin {
		...
		void testRegister(Vertx v, VertxTestContext ctx) {
			...
			store.registerUser(testEmail, testUsername, testPassword).onComplete(ar -> {
				if (ar.succeeded()) {
					assertNotNull(ar.result().getId());
					assertNotNull(ar.result().getUsername());
				...
				if (ar1.succeeded()) {
					store.getUserById(ar1.result().getId()).onComplete(ar2 -> {
						if (ar2.succeeded()) {
							...
							/* ensure body fields are not null */
							assertNotNull(ar2.result().getUsername());
							assertNotNull(ar2.result().getRole());
							assertNotNull(ar2.result().getCreatedAt());
							assertNotNull(ar2.result().getUpdatedAt());
							ctx.completeNow();
